        ...
        {!unlimited && (
          <div className="space-y-2">
            ...
            <Progress 
              ...
              style={{"--progress-background": colors.progress} as React.CSSProperties}
            />
          </div>
        )}
...
export default function UsageDisplay({ 
  ...
  const totalCost = parseFloat(dailyUsage?.totalCost || "0");

  const promptLimit = subscription.dailyPromptLimit === -1 ? 0 : subscription.dailyPromptLimit;
  const isProPlan = subscription.plan === "pro" || subscription.plan === "gold";
  const isUnlimitedPrompts = subscription.dailyPromptLimit === -1;

  // Calculate test limits based on plan (example limits)
